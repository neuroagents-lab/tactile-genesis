...
def test_vector_field_plotter_subplots(mpl_agg_backend):
    """MPLVectorFieldPlot lays out one subplot per ``subplot_titles`` entry in a single figure, sharing positions;
    the data_func returns (K, N, 3). Without ``subplot_titles`` it stays a single plot fed (N, 3)."""
    scene = gs.Scene(show_viewer=False, show_FPS=False)
    scene.add_entity(gs.morphs.Box(size=(0.1, 0.1, 0.1), pos=(0.0, 0.0, 0.5)))

    n = 9
    positions = np.c_[np.stack(np.meshgrid(np.linspace(0, 1, 3), np.linspace(0, 1, 3)), -1).reshape(-1, 2), np.zeros(n)]
    titles = ("a", "b", "c", "d")
    ...
    def grid_data():
        counter["k"] += 1
        return np.stack([(positions - 0.5) * (i + 1) * 0.1 * counter["k"] for i in range(len(titles))])

    grid_plotter = scene.start_recording(
        data_func=grid_data,
        rec_options=gs.recorders.MPLVectorFieldPlot(
            title="grid", positions=positions, normal=(0.0, 0.0, 1.0), subplot_titles=titles, show_window=False
        ),
    )
    single_plotter = scene.start_recording(
        data_func=lambda: (positions - 0.5) * 0.1,
        rec_options=gs.recorders.MPLVectorFieldPlot(
            title="single", positions=positions, normal=(0.0, 0.0, 1.0), show_window=False
        ),
    )
    ...
    assert tuple(titled) == titles  # one subplot per env, in order
    assert len(grid_plotter._quivers) == len(titles)
    assert len(single_plotter._quivers) == 1  # backward compatible single plot
    for plotter in (grid_plotter, single_plotter):
        assert plotter.get_image_array().std() > 1.0  # rendered something, not a blank canvas
