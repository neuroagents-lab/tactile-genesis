...
class SpatialCrosstalkOptionsMixin(SensorOptions[SensorT]):
    ...
    @property
    def crosstalk_enabled(self) -> bool:
...
class ContactHysteresisOptionsMixin(SensorOptions[SensorT]):
    ...
    gate latches ON when depth >= ``contact_threshold`` and releases when depth <= ``release_threshold``. The band between
    them suppresses chatter. What the gate controls is sensor-specific (ContactProbe: the boolean output;
...
class ElastomerTaxel(
    ...
    elastomer_boundary : array-like[float], shape (m, 3), optional
        Closed loop of points (link-local frame) tracing the gel's clamped housing wall -- the lateral Dirichlet
        condition (total displacement = 0 on the loop) a bonded-slab FEM would use. When set, a wall-correction
        field is solved at build from the depth-averaged bonded-layer (Darcy-Brinkman) equations with the free
        field's negated wall trace as Dirichlet data, so per step the *total* in-plane displacement vanishes at
        the wall with a physical boundary layer of width ``~ elastomer_thickness / sqrt(3)``: outward flow is
        reduced near the wall (never reversed) and the marker stretch *gradient* between the contact and the wall
        is enhanced -- the clamped-pad signature -- while the deep interior recovers the unconfined field exactly.
        Touches only the in-plane channels (the normal bulge is unchanged) and clamps the full ``compressibility``
        blend. Requires ``elastomer_thickness > 0`` (the wall-layer width is physical, set by the gel thickness)
        and a grid (FFT) layout. ``None`` (default) leaves the field unconfined. The model has no tunable shape
        parameters: wall behavior is fully determined by the loop geometry and the thickness.
    lambda_d: float
    ...
    # (m, 3) closed boundary loop; validated in model_post_init (pydantic's nested-tuple unions don't cleanly
    # disambiguate a 2D float array). None -> free-space dilation.
    elastomer_boundary: Any | None = None
    ...
    def model_post_init(self, context: Any) -> None:
        ...
        if self.elastomer_boundary is not None:
            b = np.asarray(self.elastomer_boundary, dtype=float)
            if b.ndim != 2 or b.shape[0] < 3 or b.shape[1] != 3:
                gs.raise_exception(f"elastomer_boundary must be a closed loop of shape (m>=3, 3). Got shape {b.shape}.")
            if self.elastomer_thickness <= 0.0:
                gs.raise_exception(
                    "elastomer_boundary requires elastomer_thickness > 0: the clamped-wall boundary layer width "
                    "is set by the gel thickness (h / sqrt(3)), not by a tunable parameter."
                )
