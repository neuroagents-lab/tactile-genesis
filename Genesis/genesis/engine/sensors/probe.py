...
def get_measured_bufs(
    ...
    """Return ``(measured_ring_slot, measured_scratch_T)`` for a probe sensor's update kernel.

    ``zero=True`` (default) zeroes the GT buffer and the measured ring slot first, so a kernel
    that only writes the columns of *active* probes/envs leaves the rest at zero (point-cloud
    and surface-distance rely on this). Pass ``zero=False`` only when the caller's kernel
    provably overwrites *every* cache column for *every* env (the kinematic / contact-depth
    taxel kernels write all probes' force/torque or depth, fillers included) — then the two
    full ``(cache_size, B)`` ``zero_()`` passes are dead work and dominate at high taxel counts.
    """
