...
def _func_query_contact_depth_penetration(
    ...
    for k in range(n_g):
        i_g = sensor_geoms_idx[i_b, i_s, k]
...
def _kernel_build_sensor_geom_idx(
    ...
    Parallelizes over ``(n_batches, n_sensors)``: each thread scans the collider's contact list once and, for
    every contact touching the sensor's tracked link, records the geom on the OPPOSITE side (``geom_b`` when the
    link is ``link_a``, ``geom_a`` when it is ``link_b`` -- mirroring ``_func_query_contact_depth``'s ``i_g``
    selection). Duplicates are dropped via a linear scan of the small accumulating list, so the SDF path's
    per-probe loop runs once per distinct contacting geom instead of once per contact point: it collapses the
    multicontact fan-out (tens of contacts on one pressing object -> one geom) that previously made each probe
    re-query the same SDF. Cap-overflows (count >= last dim of ``sensor_geoms_idx``) silently truncate; see the
    module-level ``_MAX_GEOMS_PER_SENSOR`` comment.
    """
    n_sensors = sensor_link_idx.shape[0]
    ...
    for i_b, i_s in qd.ndrange(n_batches, n_sensors):
        ...
        for i_c in range(n_c):
            la = collider_state.contact_data.link_a[i_c, i_b]
            lb = collider_state.contact_data.link_b[i_c, i_b]
            # Each side contributes the geom opposite the sensor link; a self-contact (link on both sides) is
            # deduped naturally below.
            for side in qd.static(range(2)):
                c_link = la if side == 0 else lb
                if c_link == link:
                    i_g = (
                    ...
                    for j in range(count):
                        if sensor_geoms_idx[i_b, i_s, j] == i_g:
                            already = True
...
def _func_query_contact_depth(
    ...
    ``geoms_info.link_idx[i_g]`` (the link owning the opposing geom -- exactly the link the old contact-list
    path reported). AABB pre-filter expands by ``max(probe_radius_gt, probe_radius_m)`` so neither branch is
    ...
    for i_p, i_b in qd.ndrange(total_n_probes, n_batches):
        ...
        if probe_radii[i_p] <= gs.qd_float(0.0):
            for j in qd.static(range(3)):
                ...
                # When measured == GT the caller populates the measured ring from GT directly, so
                # the measured scratch is never read -- skip writing it (see _update_current_timestep_data).
        ...
        for j in qd.static(range(3)):
            ...
            # When measured == GT the caller copies the measured ring straight from GT (a contiguous
            # copy, no transpose), so the measured scratch is dead -- skip the writes entirely.
            if measured_equals_gt == 0:
...
class ContactDepthProbeSensor(
    ...
    def _update_current_timestep_data(
        ...
        # zero=False: `_kernel_contact_depth_probe` writes one depth per probe for every probe
        # (fillers included), fully covering the GT buffer, and the measured ring slot is fully
        # overwritten by the transposed copy below — so the zeroing passes are dead work.
        measured, measured_cols_b = get_measured_bufs(
            shared_metadata, current_ground_truth_data_T, measured_data_timeline, zero=False
        )
...
class ContactProbeSensor(
    ...
    ``release_threshold``. When ``release_threshold`` is left unset (the default; it then falls back to ``contact_threshold``),
    the latch is degenerate and behavior matches a stateless threshold. Latch state is read from the per-branch
    return-space ring, so GT and measured branches latch independently and reset cleanly with the env (the manager
    zeros the ring on reset).
    """
...
class KinematicTaxelSensor(
    ...
    # The GT-branch transform is always a no-op here: the only `_apply_transform` overrides in the MRO
    # (ViscoelasticHysteresisMixin, SpatialCrosstalkMixin) act only when `is_measured=True`. So the base
    # pipeline can skip the GT transpose copy-back -- see `SimpleSensor._gt_branch_transform_is_identity`.
    _gt_branch_transform_is_identity: bool = True
    ...
    def _update_current_timestep_data(
        ...
        solver = shared_metadata.solver
        # zero=False: `_kernel_kinematic_taxel` writes force+torque for every probe (fillers
        # included), fully covering the GT buffer, and the measured ring slot is fully written
        # below (transposed copy, or a contiguous copy from GT when measured==GT) — so the two
        # full (cache_size, B) zeroing passes are dead work and dominate at high taxel counts.
        measured, measured_cols_b = get_measured_bufs(
            shared_metadata, current_ground_truth_data_T, measured_data_timeline, zero=False
        )
        ...
        if ground_truth_data_timeline is not None:
            gt_slot = ground_truth_data_timeline.at(0, copy=False)
            gt_slot.copy_(current_ground_truth_data_T.T)
            if measured_equals_gt:
                # Raw measured signal == raw GT (no radius noise, no gain): the kernel skipped the
                # measured writes, so populate the measured ring from the just-written GT ring slot
                # with a CONTIGUOUS (B, cache) copy instead of a transposed copy of the scratch.
                # Branch-specific transforms (hysteresis / crosstalk) still run on the measured ring
                # afterward via `_apply_transform(is_measured=True)`, so this only shares the raw value.
                measured.copy_(gt_slot)
            else:
                measured.copy_(measured_cols_b.T)
        elif measured_equals_gt:
            # No GT ring to source from: still avoid the dead scratch, transpose-copy GT directly.
