...
class TriangleMeshBVH(BVHMetadata):
    ...
    def append_sensor(self, track_link_idx: np.ndarray, solver) -> None:
        ...
        for i_l in range(int(track_link_idx.shape[0])):
            ...
            all_node_leaf_start.append(np.where(lcount > 0, lstart + leaf_offset, lstart).astype(gs.np_int))
            all_node_leaf_count.append(lcount)
            all_leaf_elem_idx.append(eidx)
            all_tri_verts.append(tri_stack.astype(gs.np_float, copy=False))
...
class SurfaceDistanceProbeSensor(
    ...
    def _draw_debug(self, context: "RasterizerContext"):
        ...
        link_pos = self._link.get_pos(env_idx).squeeze(0)
        link_quat = self._link.get_quat(env_idx).squeeze(0)
