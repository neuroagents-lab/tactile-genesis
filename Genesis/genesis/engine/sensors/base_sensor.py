...
class SimpleSensor(Sensor[OptionsT, SharedSensorContextT, SharedSensorMetadataT, DataT]):
    ...
    # When True, ``_apply_transform(is_measured=False)`` is a proven no-op for this class, so the
    # GT branch's transpose copy-back of the (transform-unchanged) GT ring slot into
    # ``current_ground_truth_data_T`` is skipped (the kernel already left the correct GT there).
    # Conservative default False -> sensors whose GT branch runs branch-symmetric transforms
    # (coordinate change, frame rotation) keep the copy-back. See ``_update_shared_cache``.
    _gt_branch_transform_is_identity: ClassVar[bool] = False
    ...
    def _update_shared_cache(
        ...
        if measured_data_timeline is None:
            ...
            # bandwidth) skip on the GT path while branch-symmetric coordinate transforms still run. When the GT
            # transform is a proven no-op (`_gt_branch_transform_is_identity`), `current_ground_truth_data_T` already
            # holds the correct GT (the kernel wrote it), so both the transform and its transpose copy-back are dead
            # work -- skipping them removes one full (cache_size, B) transpose copy per step (dominant at high taxels).
            if not cls._gt_branch_transform_is_identity:
