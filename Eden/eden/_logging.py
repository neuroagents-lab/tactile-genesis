"""Logging setup: Eden's colored formatter and rank-aware logger."""

# Code modified from: https://github.com/Genesis-Embodied-AI/Genesis/blob/main/genesis/logging/logger.py
# Copyright (c) 2025, Genesis-Embodied-AI
# Apache License 2.0
...
from genesis.logging.time_elapser import TimeElapser
...
class EdenFormatter(logging.Formatter):
    ...
    def colored_fmt(self, color):
        ...
        return f"{color}[Eden 🍏] [{self.TIME}] [{self.LEVEL}] {self.MESSAGE}{formats.RESET}"
...
class Logger:
    def __init__(self, logging_level: int | str, verbose_time: bool = False):
        if isinstance(logging_level, str):
            logging_level_str = logging_level.upper()
            logging_level = getattr(logging, logging_level_str, None)
            ...
            raise ValueError(f"Logging level must be a string or integer, got: {type(logging_level)}")

        # TODO: fix this in Genesis
        gs_logger = logging.getLogger("genesis")
        gs_logger.setLevel(logging_level)
        gs_logger.propagate = False

        self._logger = logging.getLogger("eden")
        self._logger.setLevel(logging_level)
        self._logger.propagate = False

        self._formatter = EdenFormatter(verbose_time)
